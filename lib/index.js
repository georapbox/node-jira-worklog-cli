...
Promise
  .each(Object.keys(questions), key => {
    ...
    switch (true) {
      case argv.monthly && key === 'startDate':
        rl.write(moment().startOf('month').format('YYYY-MM-DD'));
        break;
      case argv.monthly && key === 'endDate':
        rl.write(moment().endOf('month').format('YYYY-MM-DD'));
        break;
      case argv.weekly && key === 'startDate':
        rl.write(moment().startOf('week').format('YYYY-MM-DD'));
        break;
      case argv.weekly && key === 'endDate':
        rl.write(moment().endOf('week').format('YYYY-MM-DD'));
        break;
      case argv.daily && (key === 'startDate' || key === 'endDate'):
        rl.write(moment().format('YYYY-MM-DD'));
        break;
      default:
